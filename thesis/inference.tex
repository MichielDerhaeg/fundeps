...
detail in chapter~\ref{cha:entailment}.

During constraint generation the Haskell code is also elaborated into
...
expression is assigned a ``placeholder'' type (also known as unification type variable) that might
...
context $\overline{\pi}$. As we discuss later, variables $\overline{b}$ have to
be uniquely determined by functional dependencies.
...
allowed in the source text and are only used internally. We also
define type patterns $u$. These are simple monotypes that do not
...
evidence, which are dictionary variables, coercion variables, and
...
constraints schemes and equality axioms from instance declarations, as well as local
given constraints introduced by qualified types and contexts.
...
Like most procedures in this chapter, its result has a System $F_C$
