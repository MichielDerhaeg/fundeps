...
Type substitutions are denoted by $\theta$ and when applied, replace
type variables with monotypes. Evidence substitutions, denoted by $\eta$,
replace placeholder dictionary variables and coercion variables to concrete
dictionary terms and type equality coercions.
...
}
{
    ...
    \theta(\pi)}, \overline{F_{TC}(\overline{\tau}^{n}) \sim \tau_{0}}
}
\\
% TmAbs
...
\inferrule*[right=Class]
{
    ...
    &  &, \textbf{let} \; f : v_{real} = \Lambda \overline{a} \overline{a'}.
      \lambda(d : T_{TC} \; \overline{a}). \; \\ 
      &  &\hspace{3cm} \textbf{case} \; d \; \textbf{of} \;
...
Because type classes are represented by dictionaries in \systemfc ~at runtime, a
...
\inferrule*[right=Instance]
{
    ...
    \\
    P_I, d_I : S_I; \Gamma_I ; \overline{a}\overline{b} \vdash_{tm} e : [\overline{a'} \mapsto
...
Most of the heavy lifting and FD-related concepts are handled by match contexts
...
annotated with $d_I$ as evidence of this instance declaration that can be used
later on by the entailment algorithm. This is always a let-binding of a
dictionary term. For example, \texttt{instance Eq a => Eq [a]} creates a
function that takes a dictionary for \texttt{Eq a} as an argument to create a
dictionary for \texttt{Eq [a]}. Indeed, we have to provide evidence for
\texttt{Eq a} to be able to retrieve evidence for \texttt{Eq [a]}.
...
\begin{figure}
\fbox{$P; \Gamma ; a_{utch} \vdash_{tm} e : \sigma \rightsquigarrow t$}
\begin{mathpar}
\inferrule*[right=Subsumption]
{
    \Gamma \vdash_{tm} e : \tau_1 \rightsquigarrow t \mid \mathcal{C}
    \\
    ...
    \mid P_{\mathbb{E}} \mid \Gamma_{\mathbb{E}}
    \\
    P, (\overline{d : \pi}),P_{\mathbb{E}} ; a_{utch} \vDash \mathcal{C},
    (c : \tau_1 \sim \tau_2) \rightsquigarrow \bullet; \theta; \eta
}
{
    P; \Gamma ; a_{utch} \vdash_{tm} e : (\forall \overline{a}. \; \overline{\pi}
